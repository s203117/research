...
def reprediction_control():
    ...
    time = np.arange(0, predict_T+ctrl_T+1)
    plot = np.concatenate([RC_plot, ctrl])

    fig = plt.figure(figsize = (6, 5), dpi = 100)
    ax1 = fig.add_subplot(1,1,1)
    ax1.scatter(aa_t, original_signal, s = 1, zorder=1, c = 'green', label="")
    #ax1.plot(ctrl[:-1], ctrl[1:], lw = 2, zorder=2, c = 'red', label="")
    ax1.plot(plot[:-1], plot[1:], lw = 2, zorder=2, c = 'red', label="")
    ...
    ax2.plot(time, plot, lw = 2, zorder=3, c = 'red', label="")
    ax2.axvline(x=predict_T, c='gray', zorder=1)
    ax2.axhline(y=fixpoint, c='yellow', zorder=2)

    ax2.set_xlabel('t', fontsize=20)
    ax2.set_ylabel('x', fontsize=20, labelpad=-2)
    ax2.set_xlim(0, predict_T+ctrl_T)
    ax2.set_ylim(0,1.0)

    ax2.tick_params(direction = "in", labelsize=17, pad = 4, length = 6)

    fig.savefig('repre+ctrl.pdf', bbox_inches='tight', pad_inches=0.05)
    fig.savefig('repre+ctrl.png', bbox_inches='tight', pad_inches=0.05)
...
# 再帰予測 (RCの出力を次の入力に) と stability tranceformation
reprediction_control()