...
def control():
    ...
    fig = plt.figure(figsize = (12, 5), dpi = 100)
    ax1 = fig.add_subplot(1,1,1)
    ax1.plot(t, fix_Hx_plot, lw = 2, zorder=1, c = 'yellow', label="")
    ax1.plot(t, RC_plot, lw = 2, zorder=2, c = 'red', label="")
        ...
        while k < reservoir_node:
            if j != k:
                if W[j,k] != 0:
                    ...
                    #print("Win[j]",Win[j])
                    #print("W[j,k]",W[j,k])
                    A[j,k] = alpha * W[j,k] / pow(np.cosh(Win[j]*RC_fix + aaa), 2)
            k = k+1
        j = j+1
    A = A @ (I + Win@Wout)
    ...
    ax1 = fig.add_subplot(1,1,1)
    ax1.scatter(original_Hx[:-1], original_Hx[1:], s = 1, zorder=1, c = 'green', label="")
    ax1.plot(ctrl[:-1], ctrl[1:], lw = 2, zorder=2, c = 'red', label="")
    ax1.scatter(fix_Hx, fix_Hx, s = 20, zorder=2, c = 'yellow', label="")
    ax1.scatter(RC_fix, RC_fix, s = 20, zorder=2, c = 'black', label="")
    ...
    fig = plt.figure(figsize = (6, 5), dpi = 100)
    ax2 = fig.add_subplot(1,1,1)
    ax2.plot(t, fix_Hx_plot, lw = 1, zorder=1, c = 'yellow', label="")
