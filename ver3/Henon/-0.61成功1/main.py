...
def make_W():
    ...
    print("W生成 : {:.2f}[s]".format(time.perf_counter() - start))
...
def make_Win():
    ...
    print("Win生成 : {:.2f}[s]".format(time.perf_counter() - start))
...
def make_Wout():
    ...
    print("Wout学習 & システム同定 : {:.2f}[s]".format(time.perf_counter() - start))
    #print("system doutei o", o)
...
def make_reprediction():
    ...
    print("再帰予測完了 : {:.2f}[s]".format(time.perf_counter() - start))
    #print("saiki yosoku o", o)
...
def control():
    ...
    print("固定点安定化終了 : {:.2f}[s]".format(time.perf_counter() - start))
...
print("終了 : {:.2f}[s]".format(time.perf_counter() - start))